...
// Autocomplete search for emails (partial match)
exports.autocompleteEmail = async (req, res, next) => {
  try {
    const { q } = req.query;
    ...
    console.error("Autocomplete Email Error:", error);
    res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.addCompetency = async (req, res, next) => {
  try {
    ...
    if (!competency) {
      return res
        .status(404)
        .json({ message: "Kompetensi tidak ditemukan atau tidak aktif" });
    }
    ...
    if (user.competencies.includes(competencyId)) {
      return res
        .status(409)
        .json({ message: "Kompetensi sudah ditambahkan sebelumnya" });
    }
    ...
    if (user.competencies.length >= 20) {
      return res
        .status(400)
        .json({ message: "Maksimal 20 kompetensi yang dapat ditambahkan" });
    }
...
exports.updateCompetency = async (req, res, next) => {
  try {
    ...
    // Validasi index
    if (
      isNaN(competencyIndex) ||
      competencyIndex < 0 ||
      competencyIndex >= user.competencies.length
    ) {
    ...
    const existingIndex = user.competencies.findIndex(
      (comp, idx) => idx !== competencyIndex && comp.toString() === competencyId
    );

    if (existingIndex !== -1) {
      return res
        .status(409)
        .json({ message: "Kompetensi sudah ada di daftar Anda" });
    }
...
exports.setCompetencies = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(competencyIds)) {
      return res
        .status(400)
        .json({ message: "competencyIds harus berupa array" });
    }
    ...
    if (uniqueIds.length !== competencyIds.length) {
      return res
        .status(400)
        .json({ message: "Terdapat kompetensi yang duplikat" });
    }
...
exports.getUserProfile = async (req, res, next) => {
  try {
    ...
    const user = await User.findById(userId)
      .select(
        "name email role competencies createdAt updatedAt isVerified roleApproved pendingRole"
      )
      .populate("competencies", "name category description");
...
exports.searchUsersByCompetency = async (req, res, next) => {
  try {
    ...
    if (!competencyId) {
      return res
        .status(400)
        .json({ message: "competencyId parameter wajib diisi" });
    }
...
exports.exportUsers = async (req, res) => {
  try {
    ...
    const csvData = users.map((user) => ({
      ...
      "Tanggal Registrasi": user.createdAt
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=pengguna_capstation_${
